        ...
        integer :: m,n
        real(kind=r8), dimension(:,:,:), allocatable :: Adiag,Asub,Asup 
        real(kind=r8), dimension(:,:,:), allocatable :: Bdiag,Bsub,Bsup 
        real(kind=r8), dimension(:,:,:), allocatable :: D,L,U
     ...
     &          m,n, Cdiag,Csub,Csup)
        ...
        real(kind=r8), dimension(m,m,n),intent(inout)::Cdiag,Csub,Csup
...
c
     ...
     &               m,n, Cdiag,Csub,Csup)
         ...
         real(kind=r8),dimension(:,:,:),allocatable,intent(inout) ::    
     &             Cdiag,Csub,Csup
...
!        --------------------
         ...
         call cpu_time(time_1)
         Adiag = Adiag - dble(ztarget) * Bdiag;
         Asub  = Asub  - dble(ztarget) * Bsub;
         Asup  = Asup  - dble(ztarget) * Bsup;
