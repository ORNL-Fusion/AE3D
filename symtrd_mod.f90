     ...
     &                                 L, D, ipiv, U, info )
        ...
        real(kind=r8), dimension(m,m,n), intent(in) :: A,B,C
        real(kind=r8), dimension(m,m,n), intent(inout) :: L,D,U
...
!
        ...
        real(kind=r8), dimension(m,m) :: Dkm1,Dk,Lk,Ukm1
...
!  -------
        ...
        ld1 = size(Dk,1)
        call dgetrf(m,m,Dk,ld1,ipivk,linfo)
        if (linfo.ne.0) then

          if (idebug.ge.1) then
            write(*,*) 'symtrd_factor: dgetrf(D1) linfo=',linfo
          endif
...
!      ------------------
         ...
         ld1 = size(Dkm1,1)
         ld2 = size(Lk,1)
         call dgetrs( 'T', m,nrhs, Dkm1, ld1, ipivkm1, Lk,ld2,linfo)
         if (linfo.ne.0) then

          if (idebug.ge.1) then
            write(*,*) 'symtrd_factor: dgetrs, k,linfo ',k,linfo
          endif
...
!      ------------------- 
     ...
     &                    matmul(Lk(1:m,1:m),Ukm1(1:m,1:m))
          ...
          if (linfo.ne.0) then
            ...
            if (idebug.ge.1) then
              write(*,*) 'symtrd_factor: dgetrf(Dk), k,linfo ',k,linfo
            endif
     ...
     &           Brhs_inout, info )
        ...
        real(kind=r8), dimension(m,m,n),target,intent(in) :: L, D, U
        integer, dimension(m,n), target, intent(in) :: ipiv

        real(kind=r8), dimension(m,n,nrhs), intent(inout) :: Brhs_inout
        integer, intent(inout) :: info
...
!
        ...
        real(kind=r8), dimension(m,nrhs,n),target :: Brhs


        real(kind=r8), dimension(:,:,:), pointer :: Y,X
        real(kind=r8), dimension(:,:), pointer :: yk,ykm1,xk,xkp1
        real(kind=r8), dimension(:,:), pointer :: Dk,Uk,Lk
...
!    -------------------------------------
        ...
        ld1 = size(Dk,1)
        ld2 = size(xk,1)
        call dgetrs('N',m,nrhs,Dk,ld1,ipivk,  xk,ld2,linfo)
        if (linfo.ne.0) then
          if (idebug.ge.1) then
            write(*,*) 'symtrd_solve: dgetrs, k,linfo ',k,linfo
          endif
...
!          -----------------------------
     ...
     &                      matmul(Uk(1:m,1:m), xkp1(1:m,1:nrhs))
        ...
        real(kind=r8), target, dimension(m,m,n), intent(in) ::  A,B,C
        real(kind=r8), target, dimension(m,n,nrhs), intent(in) :: X
        real(kind=r8), target, dimension(m,n,nrhs), intent(inout) :: Y



        integer :: k

        real(kind=r8), dimension(:,:), pointer :: Ak, Bk, Ck
        real(kind=r8), dimension(:,:), pointer :: yk, xkm1,xk,xkp1
...
!       ------------------------------------------------
        real(kind=r8), dimension(m,m,n) :: A,B,C,L,D,U
        real(kind=r8), dimension(m,n,nrhs) :: X,Brhs,Resid,Xsolve

        integer, dimension(m,n) :: ipiv

        integer :: i,j,k, info

        real(kind=r8),dimension(m,m) :: Ak, Bkp1,Ck


        real(kind=r8) :: t1,t2,abserr,maxerr
!       ----------------
!       generate problem
!       ----------------
